...
#include <io.h>
#include <windows.h>
#define DUDE_ISATTY(fd) _isatty(fd)
#define DUDE_FILENO(f) _fileno(f)
#else
#include <unistd.h>
#define DUDE_ISATTY(fd) isatty(fd)
#define DUDE_FILENO(f) fileno(f)
#endif
...
{
    auto const handle = reinterpret_cast<HANDLE>(_get_osfhandle(DUDE_FILENO(output)));
