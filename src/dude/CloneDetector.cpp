                           ...
                           ProgressCallback const& collectCallback) -> std::vector<CloneGroup>
{
    ...
    std::vector<CandidatePair> candidates;
    {
        ...
        for (auto& pc : perPartitionCandidates)
            candidates.insert(candidates.end(), std::make_move_iterator(pc.begin()), std::make_move_iterator(pc.end()));
    }
    ...
    {
        auto work = stdexec::starts_on(sched, stdexec::just() | stdexec::bulk(stdexec::par, numWorkers, computeRange));
        stdexec::sync_wait(work);
    }
                                                   ...
                                                   std::vector<NormalizedTokenId> const& b, double threshold) -> double
{
    ...
    auto const minLcs = static_cast<size_t>(std::ceil(threshold * static_cast<double>(m + n) / 2.0));
